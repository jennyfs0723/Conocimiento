...
-- 1. RegistrarEnvio: Crea un nuevo envío con origen, destino y características.


DELIMITER $$

CREATE PROCEDURE registrar_envio(
	 IN numero_guia VARCHAR(50),
	 IN fecha_hora_recepcion DATETIME,
	 IN idCliente INT,
	 IN idConductor INT,
	 IN idVehiculo INT ,
	 IN remitente VARCHAR (50),
	 IN destinatario VARCHAR (50),
	 IN origen VARCHAR(50),
	 IN destino VARCHAR(50),
	 IN tipo_servicio VARCHAR (50),
	 IN caracteristicas VARCHAR (100),
	 IN peso DECIMAL(10,2),
	 IN dimensiones VARCHAR (50),
	 IN valor_declarado DECIMAL (10,2),
	 IN costo DECIMAL (10,2),
	 IN forma_pago VARCHAR(50),
	 IN estado_actual VARCHAR (50),
    IN instrucciones_especiales VARCHAR (50),
    IN rutaDistribucion INT)
BEGIN 
	 INSERT INTO envios(numero_guia,fecha_hora_recepcion,id_cliente,id_conductor, id_vehiculo,
	 remitente,destinatario,direccion_origen,direccion_destino, tipo_servicio, descripcion_contenido,
	 peso,dimensiones,valor_declarado,costo,forma_pago,estado_actual, instrucciones_especiales,ruta_distribucion)
	 VALUES (numero_guia,fecha_hora_recepcion,idCliente,idConductor,idVehiculo,remitente,destinatario,origen,
destino,tipo_servicio,caracteristicas,peso,dimensiones,valor_declarado,costo,forma_pago,estado_actual, 
instrucciones_especiales,rutaDistribucion);
END $$

DELIMITER;

-- llamamos el procedimiento almacenado para insertar el nuevo envio
CALL registrar_envio('G-011', '2026-03-14 12:30:00', 2,1,1, 'Carlos Ramirez', 'Liliana Caicedo', 'Cra 29 #30s-4, Medellin',
'Calle 85 No.60-82, Cartagena', 'normal', 'Envio Ropa', 12.00, '50x50x50 cm', 380000, 30000, 'efectivo',
'en transito', 'No se puede ensuciar', 1);


-- verificamos si el envio se guardo correctamente
SELECT direccion_origen AS origen, direccion_destino AS destino, descripcion_contenido AS descripcion
FROM envios
WHERE numero_guia = 'G-011';

-- 2. AsignarRutaOptima: Asigna la ruta óptima para un envío según distancia y tráfico.

DELIMITER $$

CREATE PROCEDURE AsignarRutaOptima(
    IN numeroGuia VARCHAR(50)
)
BEGIN
    DECLARE rutaOptima VARCHAR(50);
    SELECT ruta_distribucionID
    INTO rutaOptima
    FROM ruta_distribucion
    ORDER BY tiempo_estimado ASC
    LIMIT 1;
-- si la ruta es optima se asigna
    IF rutaOptima IS NOT NULL THEN
        UPDATE envios
        SET ruta_distribucion = rutaOptima
        WHERE numero_guia = numeroGuia;
-- si no hay una ruta valida no va a generar un null si no que envia este mensaje
    ELSE
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se encontró una ruta válida para este envío';
    END IF;
END $$
...
-- llamo el procedimiento mediante el numero de guia 
CALL AsignarRutaOptima('G-011');


-- consulto si se asigno ruta optima
SELECT e.numero_guia,
       ...
       r.tiempo_estimado
FROM envios e
JOIN ruta_distribucion r
  ON e.ruta_distribucion = r.ruta_distribucionID
WHERE e.numero_guia = 'G-011';

-- 3. AsignarVehiculoConductor: Asigna vehículo y conductor para un envío verificando disponibilidad.

DELIMITER $$

CREATE PROCEDURE AsignarVehiculoConductor(
    IN envioID INT,
    OUT conductorID INT,
    OUT vehiculoID INT
)
BEGIN
    DECLARE estadoConductor VARCHAR(20);
    DECLARE estadoVehiculo VARCHAR(20);

-- Traer datos del envío, conductor y vehículo asociados
    SELECT 
	 c.conductorID,
	 v.vehiculoID, 
	 c.estado_conductor, 
	 v.estado_operativo
    INTO conductorID, vehiculoID, estadoConductor, estadoVehiculo
    FROM envios e
    INNER JOIN conductores c ON e.id_conductor = c.conductorID
    INNER JOIN vehiculo v ON e.id_vehiculo = v.vehiculoID
    WHERE e.envioID = envioID;

-- Verificar disponibilidad
    IF (estadoConductor = 'disponible' AND estadoVehiculo = 'activo') THEN
-- Asignar al envío
        UPDATE envios
        SET id_conductor = conductorID,
            id_vehiculo = vehiculoID
        WHERE envioID = envioID;
    ELSE
-- Buscar otro conductor disponible
...
-- Buscar otro vehículo disponible
        SELECT v.vehiculoID
        INTO vehiculoID
        FROM vehiculo v
        WHERE v.estado_operativo = 'activo'
        LIMIT 1;

-- Asignar al envío con los nuevos recursos
        UPDATE envios
        SET id_conductor = conductorID,
            id_vehiculo = vehiculoID
        WHERE envioID = envioID;
...
CALL AsignarVehiculoConductor(1, @conductor, @vehiculo);


-- verificar si se asigno correctamente condcutor y vehiculo disponible
SELECT @conductor AS ConductorAsignado, @vehiculo AS VehiculoAsignado;

-- 4. RegistrarSeguimientoEnvio: Actualiza la posición y estado de un envío en ruta.

DELIMITER $$

CREATE PROCEDURE RegistrarSeguimientoEnvio(
    IN EnvioID INT,
    IN ConductorID INT,
    IN CentroDistribucionID INT,
    IN ubicacion VARCHAR(100),
    IN EstadoEnvio VARCHAR(50),
    IN EmpleadoResponsable VARCHAR(100),
    IN Observaciones VARCHAR(255),
    IN CoordenadasGPS VARCHAR(50)
)
BEGIN
-- Normalizar el valor para seguimiento_envios
    DECLARE EstadoSeguimiento VARCHAR(50);

    IF EstadoEnvio = 'en transito' THEN
        SET EstadoSeguimiento = 'transito';
    ELSE
        SET EstadoSeguimiento = EstadoEnvio;
    END IF;

-- Insertar en seguimiento_envios con el valor normalizado
...
-- Actualizar el estado actual en envios 
    UPDATE envios
    SET estado_actual = EstadoEnvio
    WHERE envioID = EnvioID;
END $$
...
-- llamar al procedimiento
CALL RegistrarSeguimientoEnvio(
    1,
    1,
    1,
    'Centro Medellín Norte',
    'en transito',
    'Laura Gómez',
    'Paquete salió hacia destino',
    '6.2518, -75.5636'
);

-- verificamos si se registro el seguimiento de envio
SELECT id_envio, fecha_hora, ubicacion, actividad_realizada, empleado_responsable, observaciones, coordenadas_gps
FROM seguimiento_envios
WHERE id_envio = 1;

-- 5. ProgramarMantenimientoFlota: Programa mantenimiento para vehículos según kilometraje.

DELIMITER $$

CREATE PROCEDURE ProgramarMantenimientoFlota(
    IN VehiculoID INT,
    IN KilometrajeActual DECIMAL(10,2)
)
BEGIN
    DECLARE UltimoMantenimiento DECIMAL(10,2);

-- Obtener el último kilometraje registrado para mantenimiento de ese vehículo
    SELECT MAX(kilometraje) INTO UltimoMantenimiento
    FROM mantenimiento_vehiculo
    WHERE id_vehiculo = VehiculoID;

-- Si no hay registros previos, inicializar en 0
    IF UltimoMantenimiento IS NULL THEN
        SET UltimoMantenimiento = 0;
    END IF;

-- Verificar si el vehículo superó el umbral de 10.000 km desde el último mantenimiento
    IF (KilometrajeActual - UltimoMantenimiento) >= 10000 THEN
        ...
        VALUES (
            CONCAT('MV', VehiculoID, '-', KilometrajeActual),
            VehiculoID,
            'preventivo',
            DATE_ADD(CURDATE(), INTERVAL 7 DAY), -- fecha programada
            DATE_ADD(CURDATE(), INTERVAL 7 DAY), -- misma fecha en ejecución (para cumplir NOT NULL)
            KilometrajeActual,
            'Pendiente',
            0,
            'Pendiente',
...
CALL ProgramarMantenimientoFlota(3, 160500);
	 
-- Ver todos los mantenimientos del vehículo 3
SELECT mantenimiento_vehiculoID,
       codigo_orden,
       id_vehiculo,
       tipo,
       fecha_programada,
...
FROM mantenimiento_vehiculo
WHERE id_vehiculo = 3
ORDER BY mantenimiento_vehiculoID DESC;

-- VISTAS:
-- 1. V_EnviosEnCurso: Muestra todos los envíos actualmente en tránsito.

CREATE VIEW vw_EnviosEnCurso AS 
SELECT 
	 e.envioID,
	 se.actividad_realizada
FROM envios e 
JOIN seguimiento_envios se ON se.id_envio = e.envioID
-- filtramos la actividad realizada para que solo nos muestre los envios en transito
WHERE se.actividad_realizada = 'transito';

SELECT * FROM vw_EnviosEnCurso;

-- 2. V_DisponibilidadFlota: Detalla la disponibilidad de vehículos por tipo y zona.

CREATE VIEW vw_DisponibilidadFlota AS 
SELECT 
	 v.vehiculoID,
	 v.tipo_vehiculo,
	 v.estado_operativo,
...
-- filtramos por zona medellin norte, aunque el script sin el where filtra todas las zonas	 
SELECT * FROM vw_DisponibilidadFlota WHERE zona_geografica = 'medellin norte';

-- 3. V_RendimientoConductores: Estadísticas de rendimiento de conductores por tiempo y entregas.

CREATE VIEW vw_RendimientoConductores AS
SELECT 
    c.conductorID,
    COUNT(se.id_envio) AS total_entregas,
-- hacemos la estadistica de tiempo estimado
    AVG(TIMESTAMPDIFF(HOUR, e.fecha_hora_recepcion, se.fecha_hora)) AS promedio_horas_entrega
FROM conductores c
JOIN seguimiento_envios se 
    ON c.conductorID = se.id_conductor
JOIN envios e
    ON se.id_envio = e.envioID
-- solo vamos a seleccionar los envios entregados
WHERE se.actividad_realizada = 'entregado'
GROUP BY c.conductorID;

	
SELECT 
    conductorID,
    total_entregas,
    CONCAT(ROUND(promedio_horas_entrega, 2), ' horas') AS promedio_horas_entrega
FROM vw_RendimientoConductores
ORDER BY promedio_horas_entrega ASC;

-- 4. V_EstadisticasRutas: Análisis de tiempos y costos por ruta.

CREATE VIEW vw_EstadisticasRutas AS
SELECT 
    rd.ruta_distribucionID,
    rd.zona_geografica,
    COUNT(e.envioID) AS total_envios,
    AVG(TIMESTAMPDIFF(HOUR, e.fecha_hora_recepcion, se.fecha_hora)) AS promedio_horas_entrega,
    AVG(e.costo) AS promedio_costo_envio
FROM ruta_distribucion rd
-- usamos left join para que muester todas las rutas de distribucion asi hayan valores null 
LEFT JOIN envios e 
    ON rd.ruta_distribucionID = e.ruta_distribucion
LEFT JOIN seguimiento_envios se 
    ON e.envioID = se.id_envio AND se.actividad_realizada = 'entregado'
GROUP BY rd.ruta_distribucionID, rd.zona_geografica;

-- muestra la estadistica de las rutas que estan realizadas 
SELECT * FROM vw_EstadisticasRutas ORDER BY promedio_horas_entrega ASC;

-- 5. V_IncidenciasTransporte: Registro de incidencias por tipo, vehículo y zona.

CREATE VIEW vw_incidenciastransporte AS 
SELECT 
	 v.vehiculoID,
	 v.tipo_vehiculo,
	 ids.tipo_incidente,
	 ids.ubicacion
FROM vehiculo v 
JOIN incidentes_durante_servicio ids ON v.vehiculoID = ids.vehiculo_id;

SELECT * FROM vw_IncidenciasTransporte;
